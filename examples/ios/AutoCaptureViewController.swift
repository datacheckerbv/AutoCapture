...
//  Example integration of the AutoCapture Web SDK in iOS using WKWebView.
//
//  Prerequisites:
//  - Add LocalFileSchemeHandler.swift to your project (see README.md)
//  - Add AutoCapture dist/ folder as a folder reference in Xcode
...
class AutoCaptureViewController: UIViewController, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    ...
    private func loadIndex() {
        guard let indexURL = Bundle.main.url(forResource: "index", withExtension: "html", subdirectory: "dist") else {
            print("❌ AutoCapture index.html not found in dist/")
            return
        }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case "complete":
            ...
            print("✅ AutoCapture complete: \(String(describing: payload))")
            dismiss(animated: true)
        case "error":
            let code = dict["code"] as? String ?? "unknown"
            let stack = dict["stack"] as? String ?? ""
            print("❌ AutoCapture error: \(code)\n\(stack)")
        case "cancel":
            print("ℹ️ AutoCapture user exit")
            dismiss(animated: true)
        default:
            break
        }
